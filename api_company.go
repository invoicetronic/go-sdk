...
type ApiCompanyVatGetRequest struct {
	ctx context.Context
	ApiService *CompanyAPIService
	vat string
}

func (r ApiCompanyVatGetRequest) Execute() (*Company, *http.Response, error) {
	return r.ApiService.CompanyVatGetExecute(r)
}

/*
CompanyVatGet Get a company by vat number
...
**Companies** are the entities that send and receive invoices. They are automatically created from invoice data when invoices are sent or received.
 ...
 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param vat
 @return ApiCompanyVatGetRequest
*/
func (a *CompanyAPIService) CompanyVatGet(ctx context.Context, vat string) ApiCompanyVatGetRequest {
	return ApiCompanyVatGetRequest{
		ApiService: a,
		ctx: ctx,
		vat: vat,
...
// Execute executes the request
//  @return Company
func (a *CompanyAPIService) CompanyVatGetExecute(r ApiCompanyVatGetRequest) (*Company, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "CompanyAPIService.CompanyVatGet")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/company/{vat}"
	localVarPath = strings.Replace(localVarPath, "{"+"vat"+"}", url.PathEscape(parameterValueToString(r.vat, "vat")), -1)
